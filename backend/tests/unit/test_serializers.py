...
class TestBookingSerializerValidation:
    ...
    def test_booking_within_office_hours_accepted(self, db, room, user, make_request, berlin_now):
    ...
    @pytest.mark.timezone
    def test_booking_starting_before_office_hours_rejected(self, db, room, user, make_request, utc_now):
        """Test that booking starting before 08:00 is rejected."""
        # Create a time at 07:00 Berlin time
        ...
        assert any('office hours' in str(e).lower() or '08:00' in str(e) 
                   for e in serializer.errors.get('general', []))

    @pytest.mark.timezone
    def test_booking_starting_at_office_open_accepted(self, db, room, user, make_request, utc_now):
    ...
    @pytest.mark.timezone
    def test_booking_starting_at_office_close_boundary(self, db, room, user, make_request, utc_now):
        """Test booking at the 22:00 boundary."""
        # Starting at 22:00 should be rejected
        start = utc_now + timedelta(days=1)
        ...
        serializer = BookingSerializer(data=data, context={'request': make_request()})
        assert not serializer.is_valid()
        assert any('office hours' in str(e).lower() or '22:00' in str(e)
                   for e in serializer.errors.get('general', []))

    @pytest.mark.timezone
    def test_booking_ending_after_office_hours_rejected(self, db, room, user, make_request, utc_now):
        ...
        data = {
            'room': room.id,
            'start_time': start.isoformat(),
            'end_time': (start + timedelta(hours=2)).isoformat()  # Ends at 23:00
        }
        
        serializer = BookingSerializer(data=data, context={'request': make_request()})
        assert not serializer.is_valid()
        assert any('office hours' in str(e).lower()
                   for e in serializer.errors.get('general', []))
    ...
    def test_booking_with_15_minute_duration_accepted(self, db, room, user, make_request, utc_now):
    ...
    def test_booking_with_less_than_15_minutes_rejected(self, db, room, user, make_request, utc_now):
        ...
        serializer = BookingSerializer(data=data, context={'request': make_request()})
        assert not serializer.is_valid()
        assert any('15' in str(e) or 'minute' in str(e).lower()
                   for e in serializer.errors.get('general', []))

    def test_booking_with_8_hour_duration_accepted(self, db, room, user, make_request, utc_now):
    ...
    def test_booking_exceeding_8_hours_rejected(self, db, room, user, make_request, utc_now):
        ...
        serializer = BookingSerializer(data=data, context={'request': make_request()})
        assert not serializer.is_valid()
        assert any('8' in str(e) or 'hour' in str(e).lower()
                   for e in serializer.errors.get('general', []))
    ...
    def test_serializer_converts_times_to_berlin_timezone(self, db, booking, berlin_tz):
        """Test that serializer converts times to Berlin timezone in to_representation."""
    ...
    @pytest.mark.timezone
    def test_booking_with_utc_times_converts_for_validation(self, db, room, user, make_request, utc_now):
    ...
    def test_booking_within_90_days_accepted(self, db, room, user, make_request, utc_now):
    ...
    def test_booking_exactly_90_days_accepted(self, db, room, user, make_request, utc_now):
    ...
    def test_booking_beyond_90_days_rejected(self, db, room, user, make_request, utc_now):
        ...
        serializer = BookingSerializer(data=data, context={'request': make_request()})
        assert not serializer.is_valid()
        assert any('90' in str(e) or 'days' in str(e).lower()
                   for e in serializer.errors.get('general', []))
    ...
    def test_booking_without_end_time_rejected(self, db, room, user, make_request, utc_now):
    ...
    def test_booking_with_past_start_time_rejected(self, db, room, user, make_request, utc_now):
        ...
        serializer = BookingSerializer(data=data, context={'request': make_request()})
        assert not serializer.is_valid()
        assert any('past' in str(e).lower()
                   for e in serializer.errors.get('general', []))
    ...
    def test_booking_with_end_before_start_rejected(self, db, room, user, make_request, utc_now):
        ...
        serializer = BookingSerializer(data=data, context={'request': make_request()})
        assert not serializer.is_valid()
        assert any('after' in str(e).lower()
                   for e in serializer.errors.get('general', []))
