    ...
    def clean(self):
        ...
        if self.start_time < now:
            raise ValidationError({
                'start_time': _("Das Startdatum der Buchung darf nicht in der Vergangenheit liegen.")
            })

        if self.end_time <= self.start_time:
            raise ValidationError({
                'end_time': _("Das Enddatum muss nach dem Startdatum liegen.")
            })

        max_end = now + timedelta(days=14)
        if self.end_time > max_end:
            raise ValidationError({
                'end_time': _("Das Enddatum darf nicht später als in 14 Tagen liegen.")
            })  
