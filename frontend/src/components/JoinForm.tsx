...
export default function JoinForm() {
  ...
  useEffect(() => {
    if (!key) {
      setInviteError("No invite key provided.");
      return;
    }
    fetch(`${API_URL}/api/invites/${key}/`)
      .then((res) => {
  ...
  const validationErrors = useMemo(() => validateForm(values), [username, email, password]);
  const hasErrors = Object.keys(validationErrors).length > 0;

  const handleBlur = (field: string) => setTouched((prev) => ({ ...prev, [field]: true }));
  const showFieldError = (field: keyof FormErrors) => touched[field] && validationErrors[field];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const message = firstError?.[0] || body.error || body.detail || "Join failed.";
  ...
  if (inviteError) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface-muted p-4">
        <div className="w-full max-w-sm p-8 rounded-2xl bg-surface-base shadow-soft border border-border-subtle text-center">
          <div className="flex justify-center mb-6">
            <img src="/konfequem.svg" alt="Konfequem" width="160" height="160" />
          </div>
          <div className="mb-4 rounded-xl border px-4 py-3 text-sm bg-status-danger-soft border-status-danger-border text-status-danger-text">
            <p className="font-semibold">Invalid invite</p>
            <p className="text-sm mt-1">{inviteError}</p>
          </div>
          <Link to="/login" className="text-accent-primary hover:underline text-sm">Back to login</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-surface-muted p-4">
      <div className="w-full max-w-sm p-8 rounded-2xl bg-surface-base shadow-soft border border-border-subtle">
        ...
        {orgName && (
          <div className="mb-4 text-center p-3 rounded-xl bg-surface-muted border border-border-subtle">
            <p className="text-sm text-accent-secondary/60">You&apos;re invited to join</p>
            <p className="text-lg font-semibold text-accent-secondary">{orgName}</p>
          </div>
        )}
        ...
        {serverError && (
          <div role="alert" className="flex items-start gap-3 mb-4 rounded-xl border px-4 py-3 text-sm bg-status-danger-soft border-status-danger-border text-status-danger-text">
            <div>
              <p className="font-semibold leading-tight">Join failed</p>
